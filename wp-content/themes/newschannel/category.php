			...
			<?php if ( have_posts() ) : ?>
				...
				<header class="page-header clearfix">
					<h1 class="page-title"><?php
						printf( __( 'Category Archives: %s', 'newschannel' ), '<span class="black">' . single_cat_title( '', false ) . '</span>' );
					?></h1>

					<?php
						$category_description = category_description();
						if ( ! empty( $category_description ) )
							echo apply_filters( 'category_archive_meta', '<div class="category-archive-meta clearfix">' . $category_description . '</div>' );
					?>
				</header>
