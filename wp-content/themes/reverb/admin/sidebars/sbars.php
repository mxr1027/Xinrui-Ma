...
class Dynamic_Sidebars {
	...
	public function add_metabox() {
		add_meta_box( 'dynamic-sidebar-metabox', __( 'Dynamic Sidebar', 'dynamic_sidebars' ), array( &$this, 'display_metabox' ), 'page', 'side', 'default' );
		add_meta_box( 'dynamic-sidebar-metabox', __( 'Dynamic Sidebar', 'dynamic_sidebars' ), array( &$this, 'display_metabox' ), 'post', 'side', 'default' );
		add_meta_box( 'dynamic-sidebar-metabox', __( 'Dynamic Sidebar', 'dynamic_sidebars' ), array( &$this, 'display_metabox' ), 'gallery', 'side', 'default' );
	}
...
<label for="dynamic_sidebar" style="display: block; margin-bottom: 6px;">
  <?php _e( 'Dynamic Sidebar Name', 'dynamic_sidebars' ); ?>
</label>
		...
		foreach( (array) $this->sidebars as $sidebar ) {
			...
			register_sidebar(
				array(
					...
					'description' => apply_filters( 'ds_sidebar_description', __( $meta_value . ' Widget Area', 'dynamics_sidebars' ) ),
					'before_widget' => apply_filters( 'ds_before_widget', '<div class="widget">' ),
					'after_widget' => apply_filters( 'ds_after_widget', '</div>' ),
					'before_title' => apply_filters( 'ds_before_title', '<div class="title"><h6>' ),
					'after_title' => apply_filters( 'ds_after_title', '</h6></div>' ),
