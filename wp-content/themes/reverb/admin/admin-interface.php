...
function siteoptions_add_admin() {
    ...
    $tt_page = add_theme_page('Site Options', 'Site Options', 'edit_theme_options', 'siteoptions','siteoptions_options_page');
...
<script type="text/javascript">
			...
			jQuery('#expand_options').click(function(){
				if(flip == 0){
					flip = 1;
					jQuery('#reverb_container #of-nav').hide();
					jQuery('#reverb_container #content').width(755);
					jQuery('#reverb_container .group').add('#reverb_container .group h2').show();
	
					jQuery(this).text('[-]');
					
				} else {
					flip = 0;
					jQuery('#reverb_container #of-nav').show();
					jQuery('#reverb_container #content').width(579);
					jQuery('#reverb_container .group').add('#reverb_container .group h2').hide();
					jQuery('#reverb_container .group:first').show();
					jQuery('#reverb_container #of-nav li').removeClass('current');
					jQuery('#reverb_container #of-nav li:first').addClass('current');
...
function siteoptions_machine($options) {
	...
	foreach ($options as $value) {
		...
		case 'text':
			...
			if ( $std != "") { $val = $std; }
			$output .= '<input class="of-input" name="'. $value['id'] .'" id="'. $value['id'] .'" type="'. $value['type'] .'" value=\''. addslashes( $val ) .'\' />';
		break;
