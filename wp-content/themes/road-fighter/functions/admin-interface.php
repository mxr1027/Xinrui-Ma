...
function roadfighter_optionsframework_add_admin() {
    global $query_string;

    $themename = roadfighter_get_option('of_themename');
    $shortname = roadfighter_get_option('of_shortname');

    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework') {
        if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
            $options = roadfighter_get_option('of_template');
            roadfighter_reset_options($options, 'optionsframework');
            header("Location: admin.php?page=optionsframework&reset=true");
            die;
        }
    }
    $of_page = add_theme_page($themename, 'Theme Options', 'edit_theme_options', 'optionsframework', 'roadfighter_optionsframework_options_page', 'div');

    // Add framework functionaily to the head individually
    add_action("admin_print_scripts-$of_page", 'roadfighter_load_only');
}

add_action('admin_menu', 'roadfighter_optionsframework_add_admin');
/* ----------------------------------------------------------------------------------- */
/* Options Framework Reset Function - of_reset_options */
/* ----------------------------------------------------------------------------------- */

function roadfighter_reset_options($options, $page = '') {
    global $wpdb;
    $count = 0;
    ...
    if ($page == 'optionsframework') {
        roadfighter_delete_option('of_options');
    }
}
...
function roadfighter_optionsframework_options_page() {
    $options = roadfighter_get_option('of_template');
    $themename = roadfighter_get_option('of_themename');
    ?>
    <div class="trail-notify">
        <h1>Get Road Fighter Theme PRO!</h1>
        <p style="font-size:15px; line-height: 20px;">You are using the Lite Version of Road Fighter Theme. Upgrade to Pro for extra features like Home Page Slider, Contact Page, Gallery Page, FullWidth Page Templates, Multiple Color Options, Life time theme support and much more.</p>
        <a href="http://www.inkthemes.com/wp-themes/roadfighter-wordpress-theme/" target="blank">Upgrade to Road Fighter PRO Theme here >></a>
    </div>
    ...
    <?php wp_nonce_field('theme-update-option'); ?>
            <div id="header">
                ...
                <div class="clear"></div>
                        ...
                        $return = roadfighter_optionsframework_machine($options);
                        ?>
            <div id="main">
                <div id="of-nav">
                    <ul>
...
function roadfighter_load_only() {
    ...
    function of_admin_head() {
        ...
        <script type="text/javascript" src="<?php echo get_template_directory_uri(); ?>/functions/js/tiny_mce/tiny_mce.js"></script>
        <script type="text/javascript" src="<?php echo get_template_directory_uri(); ?>/functions/js/tiny_mce_init.js"></script>  
        ...
        foreach ($options as $option) {
            if ($option['type'] == 'color' OR $option['type'] == 'typography' OR $option['type'] == 'border') {
                if ($option['type'] == 'typography' OR $option['type'] == 'border') {
                    $option_id = $option['id'];
                    $temp_color = roadfighter_get_option($option_id);
                    $option_id = $option['id'] . '_color';
                    $color = $temp_color['color'];
                } else {
                    $option_id = $option['id'];
                    $color = roadfighter_get_option($option_id);
                }
                ?>
...
/* ----------------------------------------------------------------------------------- */
/* Ajax Save Action - roadfighter_ajax_callback */
/* ----------------------------------------------------------------------------------- */
add_action('wp_ajax_of_ajax_post_action', 'roadfighter_ajax_callback');

function roadfighter_ajax_callback() {
    ...
    if ($save_type == 'upload') {
        ...
        roadfighter_update_option($clickedID, $uploaded_file['url']);
        ...
        foreach ($options as $option_array) {
            $id = $option_array['id'];
            $old_value = roadfighter_get_option($id);
            $new_value = '';
                ...
                if (is_array($type)) {
                    foreach ($type as $array) {
                        if ($array['type'] == 'text') {
                            ...
                            roadfighter_update_option($id, stripslashes($new_value));
                        }
                    }
                } elseif ($new_value == '' && $type == 'checkbox') { // Checkbox Save
                    roadfighter_update_option($id, 'false');
                } elseif ($new_value == 'true' && $type == 'checkbox') { // Checkbox Save
                    roadfighter_update_option($id, 'true');
                    ...
                    foreach ($option_options as $options_id => $options_value) {
                        ...
                        if (!isset($output[$multicheck_id])) {
                            roadfighter_update_option($multicheck_id, 'false');
                        } else {
                            roadfighter_update_option($multicheck_id, 'true');
                        }
                    }
                    ...
                    roadfighter_update_option($id, $typography_array);
                } elseif ($type == 'border') {
                    ...
                    roadfighter_update_option($id, $border_array);
...
function roadfighter_optionsframework_machine($options) {
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case 'text':
                $val = $value['std'];
                $std = roadfighter_get_option($value['id']);
                if ($std != "") {
                    $val = $std;
                }
            ...
            case 'select':
                ...
                $select_value = roadfighter_get_option($value['id']);
            ...
            case "upload":
                $value['std'] = '';
                if (isset($value['std'])) {
                    $output .= roadfighter_optionsframework_uploader_function($value['id'], $value['std'], null);
                }
                break;
            case "upload_min":

                $output .= roadfighter_optionsframework_uploader_function($value['id'], $value['std'], 'min');

                break;
            case "color":
                $val = $value['std'];
                $stored = roadfighter_get_option($value['id']);
                if ($stored != "") {
                    $val = $stored;
                }
            ...
            case "typography":
                ...
                $default = $value['std'];
                $typography_stored = roadfighter_get_option($value['id']);
            ...
            case "border":
                ...
                $default = $value['std'];
                $border_stored = roadfighter_get_option($value['id']);
        ...
        if (is_array($value['type'])) {
            foreach ($value['type'] as $array) {
                ...
                $id = $array['id'];
                $std = $array['std'];
                $saved_std = roadfighter_get_option($id);
                if ($saved_std != $std) {
                    $std = $saved_std;
                }
                $meta = $array['meta'];
...
/* ----------------------------------------------------------------------------------- */
/* OptionsFramework Uploader - roadfighter_optionsframework_uploader_function */
/* ----------------------------------------------------------------------------------- */

function roadfighter_optionsframework_uploader_function($id, $std, $mod) {
    ...
    $uploader = '';
    $upload = roadfighter_get_option($id);

    if ($mod != 'min') {
        $val = $std;
        if (roadfighter_get_option($id) != "") {
            $val = roadfighter_get_option($id);
        }
